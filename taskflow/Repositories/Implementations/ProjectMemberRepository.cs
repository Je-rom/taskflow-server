...
namespace taskflow.Repositories.Implementations
{
    public class ProjectMemberRepository(TaskFlowDbContext dbContext) : IProjectMemberRepository
    {
        ...
        {
            return await dbContext.ProjectMembers
                .FirstOrDefaultAsync(x => x.Project.Id == project.Id && x.WorkspaceMember.Id == userId);
        }
        ...
        public async Task<ProjectMember> DeleteAsync(Project project, Guid id)
        {
            var projectMember = await FindByWorkspaceMemberIdAsync(project, id);
